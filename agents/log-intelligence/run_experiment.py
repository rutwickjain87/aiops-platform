...
def save_outputs(model: dict, outputs: dict[str, str]) -> None:
    ...
    lines = [
        ...
        "",
        f"> Model: `{model['id']}`  ",
        f"> Generated by `run_experiment.py`  ",
        "",
        "---",
        "",
    ]
...
def render_report(rows: list[ModelRow], run_date: str, n_cases: int) -> str:
    lines = [
        ...
        "",
        f"> **Run date:** {run_date}  ",
        f"> **Agent:** `log-intelligence` · **Log:** `HDFS_2k.log` (2 000 lines)  ",
...
def main() -> int:
    ...
    print(f"\n{'='*60}")
    print(f"  Log Triage — Model Routing Experiment")
    ...
    print(f"\nNext steps:")
    print(f"  1. Open each file in experiments/outputs/ and review triage quality")
    print(f"  2. Fill in 'Qualitative observations' in the report")
    print(f"  3. Fill in the 'Recommendation' section with your routing decision")
    print(f"  4. git add experiments/ && git commit")
    print(f"{'='*60}\n")
    return 0
